...
if (!$wd) {
    ...
    //查询用户列表
    $sql = "SELECT user_id, nick, school, email, create_time FROM users ORDER BY 'reg_time' DESC LIMIT $start, $each_page";
    $result = pdo_query($sql);
} else {
    //有搜索内容
    $sql = "SELECT COUNT(1) FROM users WHERE (user_id LIKE '%$wd%' OR nick LIKE '%$wd%' OR realname LIKE '%$wd%')";
    $result = pdo_query($sql);
    $total_num = (int)$result[0][0];
    $total_page = ceil($total_num / $each_page);
    //查询用户列表
    $sql = "SELECT user_id, nick, school, email, create_time FROM users WHERE (user_id LIKE '%$wd%' OR nick LIKE '%$wd%' OR realname LIKE '%$wd%') ORDER BY 'reg_time' DESC LIMIT $start, $each_page";
    $result = pdo_query($sql);
}
